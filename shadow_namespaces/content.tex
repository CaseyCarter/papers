...
\begin{itemize}
\item Within each namespace \tcode{std::cpp$X$}, there will be a shadow
  namespace \tcode{$A$::$\cdots$::$B$} for each namespace
  \tcode{std::$A$::$\cdots$::$B$} that was defined in that header in \Cpp$X$.

\item For each namespace alias \tcode{std::$A$::$\cdots$::$B$} defined in that
  header in \Cpp$X$ that denotes a namespace
  \tcode{std::$C$::\brk{}$\cdots$::$D$}, there shall be
  a corresponding shadow namespace alias
  \tcode{std::cpp$X$::$A$::$\cdots$::$B$} that denotes namespace
  \tcode{std::cpp$X$::$C$::$\cdots$::$D$}.

\item Each other name $N$ in namespace \tcode{std::$A$::$\cdots$::$B$}
  defined in that header in \Cpp$X$ shall have a corresponding
  \grammarterm{using-declaration} in \tcode{std::cpp$X$::$A$::$\cdots$::$B$}:
  \begin{codeblock}
  using ::std::@$A$@::@$\cdots$@::@$B$@::@$N$@;
  \end{codeblock}

\item Each enumerator $E$ of unscoped enumeration type $T$ defined in namespace
  \tcode{std::$A$::$\cdots$::$B$} in that header in \Cpp$X$ shall have
  a corresponding \grammarterm{using-declaration}
  in \tcode{std::cpp$X$::$A$::$\cdots$::$B$}:
  \begin{codeblock}
...
on \tcode{using namespace std;}. The Standard should provide normative
