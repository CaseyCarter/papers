...
\begin{itemize}
\item New change proposal: split \tcode{<iterator>} and \tcode{<ranges>}.
\end{itemize}
...
\item Modify the specification of \tcode{view::reverse} in
  [range.reverse.adaptor] as follows:
  \begin{quote}
\pnum
...
\rSec0[isw]{Rename \libconcept{IndirectlySwappable}}

\href{https://wg21.link/p0898r3}{P0898R3 ``Standard Library Concepts''}
established a convention that symmetric two-parameter concepts -
concepts such that \tcode{C<T, U> == C<U, T>} -
have names that end in \tcode{With}. P0896's \libconcept{IndirectlySwappable} is
such a concept. While \libconcept{IndirectlySwappableWith} is a lot to type,
changing it would be consistent with the convention.

\rSec1[isw.words]{Technical specifications}

Change all occurrences of the name \libconcept{IndirectlySwappable} to
\libconcept{IndirectlySwappableWith}.
