...
> <ins>-?- `iterator_category<I>::type` is `contiguous_iterator_tag` when `I` denotes:</ins>
>
...
>
> <ins>[*Note:* Implementations of this document that also implement `basic_string_view` from ISO/IEC 14882:2017 are also encouraged to ensure that `iterator_category<I>::type` is `contiguous_iterator_tag` when `I` denotes the member type `iterator` or `const_iterator` of a specialization of `basic_string_view`.&mdash;*end note*]
...
>
> <ins>Let `a` and `b` be a dereferenceable iterators of type `I` such that `b` is reachable from `a`. `ContiguousIterator<I>` is satisfied only if:</ins>
>
> * <ins>`addressof(*(a + (b - a))) == addressof(*a) + (b - a)`</ins>
...
> <ins>-?- The member typedef-name `iterator_category` denotes `random_access_iterator_tag` if `ContiguousIterator<I>` is satisfied, and otherwise denotes the same type as `iterator_category_t<I>`.</ins>
