...
The \tcode{View} concept requires that its models are \tcode{Copyable}, which
excludes some useful \tcode{Range} types. This proposal suggests relaxing the
copyability requirement to allow \tcode{Range} types which are \tcode{Movable}
but not \tcode{Copyable} to model \tcode{View} be first-class citizens in
...
\rSec1[background]{Problem description}

%% ericniebler/range-v3@d2bd910faa75d9016f6fb124f9de46c926c49c72 added generator 20170610
%% ericniebler/range-v3@11fb1f0c6ef60a61a3eb264b5c3d0d42fc4615a2 relaxed View 20170616

P0896R4 ``The One Ranges Proposal''~(\cite{p0896}) introduced the \tcode{View}
concept into the \Cpp working draft. \tcode{View} refines \tcode{Semiregular}
which refines \tcode{Copyable} in order that \tcode{View}s may be used in ways
similar to \tcode{Iterator}s which also refine \tcode{Semiregular}.
\tcode{Semiregular}ity combined with the constant-time complexity requirement on
\tcode{View} and \tcode{Iterator} operations allows them to copied freely by
range adaptors and the range adaptor closure object machinery used in
\tcode{View} composition. Since general \tcode{Range} types aren't required to
support such usage, they are in some sense second-class citizens in \tcode{View}
composition.
